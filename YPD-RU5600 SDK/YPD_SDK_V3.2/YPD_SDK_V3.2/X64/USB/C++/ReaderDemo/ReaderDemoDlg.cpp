...
void CReaderDemoDlg::OnBnClickedButtonOpen()
{
	...
	if(CFHid_OpenDevice(((CComboBox*)GetDlgItem(IDC_COMBO1))->GetCurSel()) == FALSE)
	{
		ShowString("Failed!\r\n");
		return;
	}
	if (CFHid_GetDeviceSystemInfo(0xFF,arrBuffer) == false) //failed
	{
...
void CReaderDemoDlg::OnBnClickedButton10()
{
	...
	CFHid_SetCallback(Dev_Callback); //Callback with Json output
	ShowString("Success\r\n"); 
...
void CReaderDemoDlg::OnBnClickedButton30()
{
	...
	int iHidNum = CFHid_GetUsbCount();
	for(int i=0; i<iHidNum; i++)
	{
		CFHid_GetUsbInfo(i, arrHidName);
