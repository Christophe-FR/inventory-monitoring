...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.ComCtrls, CFHidApi;
...
begin
    if ComboBox1.ItemIndex >= 0 then
      if CFHid_OpenDevice(ComboBox1.ItemIndex) then
      begin
          EditShow.Lines.Add('Open Success');
          if CFHid_GetDeviceSystemInfo($FF, @(pucSystemInfo[0])) then
            begin
...
begin    //WriteTag
    ...
    if CFHid_WriteCardG2($FF,@(Password[0]),Mem,WordPtr,Writelen,@(Writedata[0])) then
        begin
            ShowList('Success');
        end
    else
...
begin
    ShowList('ActiveMode');
	CFHid_SetCallback(ActiveCallback);
...
begin
    ...
    if CFHid_InventoryG2($FF,@(pBuffer[0]),@(Totallen[0]),@(CardNum[0])) then
        begin
...
begin
  ComboBox1.Items.Clear;
	iHidNum := CFHid_GetUsbCount();
  for i := 0 to iHidNum-1 do
         begin
           CFHid_GetUsbInfo(i, @(arrHidName[0]));
           str:= '';
...
begin
    ...
    DevParamAddr := 5;
	  ...
	  if CFHid_ReadDeviceOneParam($FF,DevParamAddr,@(Value[0])) then
        begin
...
begin
    ...
    Value := Ord(ComboBox2.ItemIndex);
	  if CFHid_SetDeviceOneParam($FF,DevParamAddr,Value) then
        begin
            ShowList('Success');
        end
    else
